...
public abstract class ServerProxy implements Runnable {
	...
	private String getAddress(String host, String request) {
		try {
			return String.format(Locale.getDefault(),"http://%s:%d/%s", host, port, URLEncoder.encode(request, "UTF-8"));
	...
	protected abstract static class ProxyTask implements Runnable {
